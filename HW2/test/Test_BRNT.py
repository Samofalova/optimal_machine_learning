...
Original file is located at
    https://colab.research.google.com/drive/1QbErVqzZiJZhSjcOa6LZiMhsJd5yVzQ_
"""
...
Минимум линейной аппроксимации достигается в точке {0}\
 при значении {0}\n\
Максимум линейной аппроксимации достигается в точке {0}\
 при значении {0}\n\n\
Минимум полиномиальной аппроксимации достигается в точке {P_pol_min[0]}\
 при значении {P_pol_min[1]}\n\
Максимум полиномиальной аппроксимации достигается в точке {P_pol_max[1]}\
 при значении {P_pol_max[0]}\n\n\
...
BFGS_log_min = bfgs(f, f1, x0=0.9, extreme_type='min')
...
def F(x):
    return x**2+5
BrantMethod(F, limits=[-1, 1])

def F(x):
    return x**3-3*sin(x)
BrantMethod(F, limits=[0, 1])
...
BrantMethod(F, limits=[-6, -4])

def F(x):
    return x+1/x**2
BrantMethod(F, limits=[1, 2])

def F(x):
    return exp(x)-1/3*x**3+2*x
BrantMethod(F, limits=[-2.5, -1])

def F(x):
    return x**2-2*x-2*cos(x)
BrantMethod(F, limits=[-0.5, 1])

def F(x):
    return x**(exp(0.1*x)*sin(x))
BrantMethod(F, limits=[2.5, 14])

def F(x):
    return exp(x)-1-x-x**2/2-x**3/6
BrantMethod(F, limits=[-5, 5])

def F(x):
    return 5*x**5+4*x**4-12*x**3+11*x**2-2*x+1
BrantMethod(F, limits=[-0.5, 0.5])

def F(x):
    return -(ln(x-2))**2 + (ln(10-x))**2-x**(1/5)
BrantMethod(F, limits=[6, 9.9])
...
def F(x):
    return 0.2*x*ln(x)+(x-2.3)**2
BrantMethod(F, limits=[0.5, 2.5])

def F(x):
    return -1/((x-1)**2)*(log(x)-2*(x-1)/(x+1))
BrantMethod(F, limits=[1.5, 4.5])
